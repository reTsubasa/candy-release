...
verifier=$root/.github/scripts/verify-core-cloud-module.sh
temporary=$(mktemp -d "${TMPDIR:-/tmp}/candy-core-cloud-module.XXXXXX")
...
jq -n --arg sha "$module_sha" --argjson size "$module_size" '{
  schema_version:1,
  release_kind:"candy-core-cloud-module",
  ...
  artifact:{kind:"shared-module",name:"libcandy_core_cloud.so",sha256:$sha,size_bytes:$size,target:"x86_64-unknown-linux-gnu",target_os:"linux",target_arch:"x86_64",libc:"glibc"}
...
printf '%s\n' 'Core Cloud module verifier tests passed'
