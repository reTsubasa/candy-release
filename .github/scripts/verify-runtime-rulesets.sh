...
tmp=$(mktemp -d "${TMPDIR:-/tmp}/candy-release-rulesets.XXXXXX")
...
jq -e '
  ...
  (.providers.cn_ip.source_url | type == "string" and startswith("https://")) and
  (.providers.cn_ip.source_sha256 | type == "string" and test("^[0-9a-f]{64}$")) and
  (.providers.cn_ip.installed_sha256 | type == "string" and test("^[0-9a-f]{64}$")) and
  (.providers.cn_ip.entries.total | type == "number" and floor == . and . > 0) and
  (.providers.cn_ip.entries.ipv4 | type == "number" and floor == . and . > 0) and
  (.providers.cn_ip.entries.ipv6 | type == "number" and floor == . and . > 0) and
  (.providers.gfwlist.source_url | type == "string" and startswith("https://")) and
  (.providers.gfwlist.source_sha256 | type == "string" and test("^[0-9a-f]{64}$")) and
  (.providers.gfwlist.installed_sha256 | type == "string" and test("^[0-9a-f]{64}$")) and
  (.providers.gfwlist.entries.total | type == "number" and floor == . and . > 0)
...
[ "$manifest_cidr_total" -eq $((manifest_cidr_v4 + manifest_cidr_v6)) ] ||
	fail "China IP manifest total does not equal IPv4 plus IPv6"
[ "$cidr_total" -eq "$manifest_cidr_total" ] || fail "China IP total differs from manifest"
[ "$cidr_v4" -eq "$manifest_cidr_v4" ] || fail "China IP IPv4 count differs from manifest"
[ "$cidr_v6" -eq "$manifest_cidr_v6" ] || fail "China IP IPv6 count differs from manifest"
[ "$domain_count" -eq "$manifest_domain_count" ] || fail "GFWList count differs from manifest"
...
[ "$(sha256_file "$cidr_file")" = "$manifest_cidr_sha" ] || fail "China IP file differs from manifest digest"
[ "$(sha256_file "$domain_file")" = "$manifest_domain_sha" ] || fail "GFWList file differs from manifest digest"
...
while read -r expected name extra; do
	...
	[ "$(sha256_file "$rulesets/$name")" = "$(printf '%s' "$expected" | tr 'A-F' 'a-f')" ] ||
		fail "$name does not match SHA256SUMS"
	expected_entries=$((expected_entries + 1))
done < "$checksum_file"
[ "$expected_entries" -eq 2 ] || fail "SHA256SUMS must cover exactly the two provider files"

printf '%s\n' "Verified Runtime rulesets: cn-ip=$cidr_total (IPv4=$cidr_v4 IPv6=$cidr_v6), gfwlist=$domain_count"
