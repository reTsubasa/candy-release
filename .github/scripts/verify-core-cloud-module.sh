...
case "$target" in
	...
	*) fail "unsupported Cloud module target: $target" ;;
esac
...
jq -e \
  ...
  .release_kind == "candy-core-cloud-module" and
  ...
  .artifact.kind == "shared-module" and
...
printf '%s\n' "verified Candy Core Cloud module $version for $target"
